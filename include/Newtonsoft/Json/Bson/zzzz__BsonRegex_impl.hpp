#pragma once
#include "Newtonsoft/Json/Bson/zzzz__BsonToken_impl.hpp"
#include "Newtonsoft/Json/Bson/zzzz__BsonRegex_def.hpp"
#include "Newtonsoft/Json/Bson/zzzz__BsonString_def.hpp"
#include "Newtonsoft/Json/Bson/zzzz__BsonType_def.hpp"
//  Writing Method size for method: Newtonsoft::Json::Bson::BsonRegex.get_Pattern
template<>

struct ::il2cpp_utils::il2cpp_type_check::MetadataGetter<static_cast<Newtonsoft::Json::Bson::BsonString (Newtonsoft::Json::Bson::BsonRegex::*)()>(&Newtonsoft::Json::Bson::BsonRegex::get_Pattern)> {
  constexpr static std::size_t size = 0x8;
  constexpr static std::size_t addrs = 0x2550430;

  inline static const ::MethodInfo* methodInfo() {
    static auto* ___internal_method = THROW_UNLESS((::il2cpp_utils::FindMethod(
                            ::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_class<Newtonsoft::Json::Bson::BsonRegex>::get(),
                            "get_Pattern",
                            std::vector<Il2CppClass*>{},
                            ::std::vector<const Il2CppType*>{}
                        )));
    return ___internal_method;
  }
};
//  Writing Method size for method: Newtonsoft::Json::Bson::BsonRegex.set_Pattern
template<>

struct ::il2cpp_utils::il2cpp_type_check::MetadataGetter<static_cast<void (Newtonsoft::Json::Bson::BsonRegex::*)(Newtonsoft::Json::Bson::BsonString)>(&Newtonsoft::Json::Bson::BsonRegex::set_Pattern)> {
  constexpr static std::size_t size = 0x8;
  constexpr static std::size_t addrs = 0x2550438;

  inline static const ::MethodInfo* methodInfo() {
    static auto* ___internal_method = THROW_UNLESS((::il2cpp_utils::FindMethod(
                            ::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_class<Newtonsoft::Json::Bson::BsonRegex>::get(),
                            "set_Pattern",
                            std::vector<Il2CppClass*>{},
                            ::std::vector<const Il2CppType*>{::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_type<Newtonsoft::Json::Bson::BsonString>::get()}
                        )));
    return ___internal_method;
  }
};
//  Writing Method size for method: Newtonsoft::Json::Bson::BsonRegex.get_Options
template<>

struct ::il2cpp_utils::il2cpp_type_check::MetadataGetter<static_cast<Newtonsoft::Json::Bson::BsonString (Newtonsoft::Json::Bson::BsonRegex::*)()>(&Newtonsoft::Json::Bson::BsonRegex::get_Options)> {
  constexpr static std::size_t size = 0x8;
  constexpr static std::size_t addrs = 0x2550440;

  inline static const ::MethodInfo* methodInfo() {
    static auto* ___internal_method = THROW_UNLESS((::il2cpp_utils::FindMethod(
                            ::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_class<Newtonsoft::Json::Bson::BsonRegex>::get(),
                            "get_Options",
                            std::vector<Il2CppClass*>{},
                            ::std::vector<const Il2CppType*>{}
                        )));
    return ___internal_method;
  }
};
//  Writing Method size for method: Newtonsoft::Json::Bson::BsonRegex.set_Options
template<>

struct ::il2cpp_utils::il2cpp_type_check::MetadataGetter<static_cast<void (Newtonsoft::Json::Bson::BsonRegex::*)(Newtonsoft::Json::Bson::BsonString)>(&Newtonsoft::Json::Bson::BsonRegex::set_Options)> {
  constexpr static std::size_t size = 0x8;
  constexpr static std::size_t addrs = 0x2550448;

  inline static const ::MethodInfo* methodInfo() {
    static auto* ___internal_method = THROW_UNLESS((::il2cpp_utils::FindMethod(
                            ::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_class<Newtonsoft::Json::Bson::BsonRegex>::get(),
                            "set_Options",
                            std::vector<Il2CppClass*>{},
                            ::std::vector<const Il2CppType*>{::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_type<Newtonsoft::Json::Bson::BsonString>::get()}
                        )));
    return ___internal_method;
  }
};
//  Writing Method size for method: Newtonsoft::Json::Bson::BsonRegex._ctor
template<>

struct ::il2cpp_utils::il2cpp_type_check::MetadataGetter<static_cast<void (Newtonsoft::Json::Bson::BsonRegex::*)(::StringW, ::StringW)>(&Newtonsoft::Json::Bson::BsonRegex::_ctor)> {
  constexpr static std::size_t size = 0xbc;
  constexpr static std::size_t addrs = 0x2550450;

  inline static const ::MethodInfo* methodInfo() {
    static auto* ___internal_method = THROW_UNLESS((::il2cpp_utils::FindMethod(
                            ::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_class<Newtonsoft::Json::Bson::BsonRegex>::get(),
                            ".ctor",
                            std::vector<Il2CppClass*>{},
                            ::std::vector<const Il2CppType*>{::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_type<::StringW>::get(), ::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_type<::StringW>::get()}
                        )));
    return ___internal_method;
  }
};
//  Writing Method size for method: Newtonsoft::Json::Bson::BsonRegex.get_Type
template<>

struct ::il2cpp_utils::il2cpp_type_check::MetadataGetter<static_cast<Newtonsoft::Json::Bson::BsonType (Newtonsoft::Json::Bson::BsonRegex::*)()>(&Newtonsoft::Json::Bson::BsonRegex::get_Type)> {
  constexpr static std::size_t size = 0x8;
  constexpr static std::size_t addrs = 0x255050c;

  inline static const ::MethodInfo* methodInfo() {
    
                            static auto* ___internal_method = THROW_UNLESS(::il2cpp_utils::ResolveVtableSlot(
                                classof(Newtonsoft::Json::Bson::BsonRegex),
                                 ::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_class<Newtonsoft::Json::Bson::BsonRegex>::get(),
                                  4
                                ));
    return ___internal_method;
  }
};
constexpr void Newtonsoft::Json::Bson::BsonRegex::__set__Pattern_k__BackingField(Newtonsoft::Json::Bson::BsonString value)  {
::cordl_internals::setInstanceField<Newtonsoft::Json::Bson::BsonString, 0x20>(this->::bs_hook::Il2CppWrapperType::instance, std::forward<Newtonsoft::Json::Bson::BsonString>(value));
}
constexpr Newtonsoft::Json::Bson::BsonString Newtonsoft::Json::Bson::BsonRegex::__get__Pattern_k__BackingField() const {
return ::cordl_internals::getInstanceField<Newtonsoft::Json::Bson::BsonString, 0x20>(this->::bs_hook::Il2CppWrapperType::instance);
}
constexpr void Newtonsoft::Json::Bson::BsonRegex::__set__Options_k__BackingField(Newtonsoft::Json::Bson::BsonString value)  {
::cordl_internals::setInstanceField<Newtonsoft::Json::Bson::BsonString, 0x28>(this->::bs_hook::Il2CppWrapperType::instance, std::forward<Newtonsoft::Json::Bson::BsonString>(value));
}
constexpr Newtonsoft::Json::Bson::BsonString Newtonsoft::Json::Bson::BsonRegex::__get__Options_k__BackingField() const {
return ::cordl_internals::getInstanceField<Newtonsoft::Json::Bson::BsonString, 0x28>(this->::bs_hook::Il2CppWrapperType::instance);
}
 Newtonsoft::Json::Bson::BsonString Newtonsoft::Json::Bson::BsonRegex::get_Pattern()  {
static auto* ___internal_method = THROW_UNLESS((::il2cpp_utils::FindMethod(
                            ::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_class<Newtonsoft::Json::Bson::BsonRegex>::get(),
                            "get_Pattern",
                            std::vector<Il2CppClass*>{},
                            ::std::vector<const Il2CppType*>{}
                        )));
return ::cordl_internals::RunMethodRethrow<Newtonsoft::Json::Bson::BsonString, false>(const_cast<void*>(instance), ___internal_method);
}
 void Newtonsoft::Json::Bson::BsonRegex::set_Pattern(Newtonsoft::Json::Bson::BsonString value)  {
static auto* ___internal_method = THROW_UNLESS((::il2cpp_utils::FindMethod(
                            ::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_class<Newtonsoft::Json::Bson::BsonRegex>::get(),
                            "set_Pattern",
                            std::vector<Il2CppClass*>{},
                            ::std::vector<const Il2CppType*>{::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_type<Newtonsoft::Json::Bson::BsonString>::get()}
                        )));
return ::cordl_internals::RunMethodRethrow<void, false>(const_cast<void*>(instance), ___internal_method, value);
}
 Newtonsoft::Json::Bson::BsonString Newtonsoft::Json::Bson::BsonRegex::get_Options()  {
static auto* ___internal_method = THROW_UNLESS((::il2cpp_utils::FindMethod(
                            ::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_class<Newtonsoft::Json::Bson::BsonRegex>::get(),
                            "get_Options",
                            std::vector<Il2CppClass*>{},
                            ::std::vector<const Il2CppType*>{}
                        )));
return ::cordl_internals::RunMethodRethrow<Newtonsoft::Json::Bson::BsonString, false>(const_cast<void*>(instance), ___internal_method);
}
 void Newtonsoft::Json::Bson::BsonRegex::set_Options(Newtonsoft::Json::Bson::BsonString value)  {
static auto* ___internal_method = THROW_UNLESS((::il2cpp_utils::FindMethod(
                            ::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_class<Newtonsoft::Json::Bson::BsonRegex>::get(),
                            "set_Options",
                            std::vector<Il2CppClass*>{},
                            ::std::vector<const Il2CppType*>{::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_type<Newtonsoft::Json::Bson::BsonString>::get()}
                        )));
return ::cordl_internals::RunMethodRethrow<void, false>(const_cast<void*>(instance), ___internal_method, value);
}
// Ctor Parameters [CppParam { name: "pattern", ty: "::StringW", modifiers: "", def_value: None }, CppParam { name: "options", ty: "::StringW", modifiers: "", def_value: None }]
 Newtonsoft::Json::Bson::BsonRegex::BsonRegex(::StringW pattern, ::StringW options)  : Newtonsoft::Json::Bson::BsonToken(THROW_UNLESS(::il2cpp_utils::New<BsonRegex>(pattern, options))) {}
 void Newtonsoft::Json::Bson::BsonRegex::_ctor(::StringW pattern, ::StringW options)  {
static auto* ___internal_method = THROW_UNLESS((::il2cpp_utils::FindMethod(
                            ::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_class<Newtonsoft::Json::Bson::BsonRegex>::get(),
                            ".ctor",
                            std::vector<Il2CppClass*>{},
                            ::std::vector<const Il2CppType*>{::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_type<::StringW>::get(), ::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_type<::StringW>::get()}
                        )));
return ::cordl_internals::RunMethodRethrow<void, false>(const_cast<void*>(instance), ___internal_method, pattern, options);
}
 Newtonsoft::Json::Bson::BsonType Newtonsoft::Json::Bson::BsonRegex::get_Type()  {
static auto* ___internal_method = THROW_UNLESS((::il2cpp_utils::FindMethod(
                            ::il2cpp_utils::il2cpp_type_check::il2cpp_no_arg_class<Newtonsoft::Json::Bson::BsonRegex>::get(),
                            "get_Type",
                            std::vector<Il2CppClass*>{},
                            ::std::vector<const Il2CppType*>{}
                        )));
return ::cordl_internals::RunMethodRethrow<Newtonsoft::Json::Bson::BsonType, false>(const_cast<void*>(instance), ___internal_method);
}
