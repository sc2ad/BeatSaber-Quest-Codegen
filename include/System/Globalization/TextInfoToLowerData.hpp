// Autogenerated from CppHeaderCreator
// Created by Sc2ad
// =========================================================================
#pragma once
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Completed includes
// Type namespace: System.Globalization
namespace System::Globalization {
  // Size: 0x10
  #pragma pack(push, 1)
  // Autogenerated type: System.Globalization.TextInfoToLowerData
  class TextInfoToLowerData : public ::Il2CppObject {
    public:
    // Creating value type constructor for type: TextInfoToLowerData
    TextInfoToLowerData() noexcept {}
    // Get static field: static public readonly System.Char[] range_00c0_0556
    static ::Array<::Il2CppChar>* _get_range_00c0_0556();
    // Set static field: static public readonly System.Char[] range_00c0_0556
    static void _set_range_00c0_0556(::Array<::Il2CppChar>* value);
    // Get static field: static public readonly System.Char[] range_10a0_10c5
    static ::Array<::Il2CppChar>* _get_range_10a0_10c5();
    // Set static field: static public readonly System.Char[] range_10a0_10c5
    static void _set_range_10a0_10c5(::Array<::Il2CppChar>* value);
    // Get static field: static public readonly System.Char[] range_1e00_1ffc
    static ::Array<::Il2CppChar>* _get_range_1e00_1ffc();
    // Set static field: static public readonly System.Char[] range_1e00_1ffc
    static void _set_range_1e00_1ffc(::Array<::Il2CppChar>* value);
    // Get static field: static public readonly System.Char[] range_2160_216f
    static ::Array<::Il2CppChar>* _get_range_2160_216f();
    // Set static field: static public readonly System.Char[] range_2160_216f
    static void _set_range_2160_216f(::Array<::Il2CppChar>* value);
    // Get static field: static public readonly System.Char[] range_24b6_24cf
    static ::Array<::Il2CppChar>* _get_range_24b6_24cf();
    // Set static field: static public readonly System.Char[] range_24b6_24cf
    static void _set_range_24b6_24cf(::Array<::Il2CppChar>* value);
    // Get static field: static public readonly System.Char[] range_2c00_2c2e
    static ::Array<::Il2CppChar>* _get_range_2c00_2c2e();
    // Set static field: static public readonly System.Char[] range_2c00_2c2e
    static void _set_range_2c00_2c2e(::Array<::Il2CppChar>* value);
    // Get static field: static public readonly System.Char[] range_2c60_2ce2
    static ::Array<::Il2CppChar>* _get_range_2c60_2ce2();
    // Set static field: static public readonly System.Char[] range_2c60_2ce2
    static void _set_range_2c60_2ce2(::Array<::Il2CppChar>* value);
    // Get static field: static public readonly System.Char[] range_a640_a696
    static ::Array<::Il2CppChar>* _get_range_a640_a696();
    // Set static field: static public readonly System.Char[] range_a640_a696
    static void _set_range_a640_a696(::Array<::Il2CppChar>* value);
    // Get static field: static public readonly System.Char[] range_a722_a78b
    static ::Array<::Il2CppChar>* _get_range_a722_a78b();
    // Set static field: static public readonly System.Char[] range_a722_a78b
    static void _set_range_a722_a78b(::Array<::Il2CppChar>* value);
    // static private System.Void .cctor()
    // Offset: 0x19FB538
    static void _cctor();
  }; // System.Globalization.TextInfoToLowerData
  #pragma pack(pop)
  // Writing MetadataGetter for method: TextInfoToLowerData::_cctor
  // Il2CppName: .cctor
  template<>
  struct ::il2cpp_utils::il2cpp_type_check::MetadataGetter<static_cast<void (*)()>(&TextInfoToLowerData::_cctor)> {
    const MethodInfo* get() {
      return ::il2cpp_utils::FindMethod(classof(TextInfoToLowerData*), ".cctor", std::vector<Il2CppClass*>(), ::std::vector<const Il2CppType*>{});
    }
  };
}
#include "extern/beatsaber-hook/shared/utils/il2cpp-type-check.hpp"
DEFINE_IL2CPP_ARG_TYPE(System::Globalization::TextInfoToLowerData*, "System.Globalization", "TextInfoToLowerData");
