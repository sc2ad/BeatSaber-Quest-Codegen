...
namespace GlobalNamespace {
  ...
  class DisappearingArrowControllerBase_1 : public UnityEngine::MonoBehaviour {
    static_assert((!std::is_complete_v<std::remove_pointer_t<T>> || std::is_base_of_v<GlobalNamespace::IGameNoteControllerInitializable_1<T>, std::remove_pointer_t<T>>) && (!std::is_complete_v<std::remove_pointer_t<T>> || std::is_base_of_v<GlobalNamespace::IGameNoteTypeProvider, std::remove_pointer_t<T>>));
    public:
