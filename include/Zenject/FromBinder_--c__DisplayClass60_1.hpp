// Autogenerated from CppHeaderCreator
// Created by Sc2ad
// =========================================================================
#pragma once
#pragma pack(push, 8)
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: Zenject.FromBinder
#include "Zenject/FromBinder.hpp"
// Completed includes
// Begin forward declares
// Forward declaring namespace: System
namespace System {
  // Forward declaring type: Type
  class Type;
}
// Forward declaring namespace: System::Collections::Generic
namespace System::Collections::Generic {
  // Forward declaring type: IEnumerable`1<T>
  template<typename T>
  class IEnumerable_1;
}
// Forward declaring namespace: Zenject
namespace Zenject {
  // Forward declaring type: InjectContext
  class InjectContext;
}
// Completed forward declares
// Type namespace: Zenject
namespace Zenject {
  // Autogenerated type: Zenject.FromBinder/<>c__DisplayClass60_1
  class FromBinder::$$c__DisplayClass60_1 : public ::Il2CppObject {
    public:
    // public System.Type concreteType
    // Offset: 0x10
    System::Type* concreteType;
    // public Zenject.FromBinder/<>c__DisplayClass60_0 CS$<>8__locals1
    // Offset: 0x18
    Zenject::FromBinder::$$c__DisplayClass60_0* CS$$$8__locals1;
    // System.Collections.Generic.IEnumerable`1<System.Object> <FromComponentsInChildrenBase>b__1(Zenject.InjectContext ctx)
    // Offset: 0x129B8CC
    System::Collections::Generic::IEnumerable_1<::Il2CppObject*>* $FromComponentsInChildrenBase$b__1(Zenject::InjectContext* ctx);
    // public System.Void .ctor()
    // Offset: 0x129B8C4
    // Implemented from: System.Object
    // Base method: System.Void Object::.ctor()
    static FromBinder::$$c__DisplayClass60_1* New_ctor();
  }; // Zenject.FromBinder/<>c__DisplayClass60_1
  check_size<sizeof(FromBinder::$$c__DisplayClass60_1), 24 + sizeof(void*) + ((24 + sizeof(void*)) % 8 != 0 ? (8 - (24 + sizeof(void*)) % 8) : 0)> __Zenject_FromBinder_$$c__DisplayClass60_1SizeCheck;
}
#include "extern/beatsaber-hook/shared/utils/il2cpp-type-check.hpp"
DEFINE_IL2CPP_ARG_TYPE(Zenject::FromBinder::$$c__DisplayClass60_1*, "Zenject", "FromBinder/<>c__DisplayClass60_1");
#pragma pack(pop)
