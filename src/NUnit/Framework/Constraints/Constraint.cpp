// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: NUnit.Framework.Constraints.Constraint
#include "NUnit/Framework/Constraints/Constraint.hpp"
// Including type: System.Lazy`1
#include "System/Lazy_1.hpp"
// Including type: NUnit.Framework.Constraints.ConstraintBuilder
#include "NUnit/Framework/Constraints/ConstraintBuilder.hpp"
// Including type: NUnit.Framework.Constraints.ConstraintResult
#include "NUnit/Framework/Constraints/ConstraintResult.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: NUnit.Framework.Constraints.Constraint..ctor
NUnit::Framework::Constraints::Constraint* NUnit::Framework::Constraints::Constraint::New_ctor(::Array<::CsObject*>* args) {
  return (Constraint*)THROW_UNLESS(il2cpp_utils::New("NUnit.Framework.Constraints", "Constraint", args));
}
// Autogenerated method: NUnit.Framework.Constraints.Constraint.get_DisplayName
::CsString* NUnit::Framework::Constraints::Constraint::get_DisplayName() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_DisplayName"));
}
// Autogenerated method: NUnit.Framework.Constraints.Constraint.get_Arguments
::Array<::CsObject*>* NUnit::Framework::Constraints::Constraint::get_Arguments() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::Array<::CsObject*>*>(this, "get_Arguments"));
}
// Autogenerated method: NUnit.Framework.Constraints.Constraint.set_Arguments
void NUnit::Framework::Constraints::Constraint::set_Arguments(::Array<::CsObject*>* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_Arguments", value));
}
// Autogenerated method: NUnit.Framework.Constraints.Constraint.get_Builder
NUnit::Framework::Constraints::ConstraintBuilder* NUnit::Framework::Constraints::Constraint::get_Builder() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<NUnit::Framework::Constraints::ConstraintBuilder*>(this, "get_Builder"));
}
// Autogenerated method: NUnit.Framework.Constraints.Constraint.GetStringRepresentation
::CsString* NUnit::Framework::Constraints::Constraint::GetStringRepresentation() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "GetStringRepresentation"));
}
// Autogenerated method: NUnit.Framework.Constraints.Constraint._displayable
::CsString* NUnit::Framework::Constraints::Constraint::_displayable(::CsObject* o) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>("NUnit.Framework.Constraints", "Constraint", "_displayable", o));
}
// Autogenerated method: NUnit.Framework.Constraints.Constraint.<.ctor>b__0
::CsString* NUnit::Framework::Constraints::Constraint::$_ctor$b__0() {
  return THROW_UNLESS((il2cpp_utils::RunMethod<::CsString*>(this, "<.ctor>b__0")));
}
// Autogenerated method: NUnit.Framework.Constraints.Constraint.get_Description
::CsString* NUnit::Framework::Constraints::Constraint::get_Description() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_Description"));
}
// Autogenerated method: NUnit.Framework.Constraints.Constraint.set_Builder
void NUnit::Framework::Constraints::Constraint::set_Builder(NUnit::Framework::Constraints::ConstraintBuilder* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_Builder", value));
}
// Autogenerated method: NUnit.Framework.Constraints.Constraint.ApplyTo
NUnit::Framework::Constraints::ConstraintResult* NUnit::Framework::Constraints::Constraint::ApplyTo(::CsObject* actual) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<NUnit::Framework::Constraints::ConstraintResult*>(this, "ApplyTo", actual));
}
// Autogenerated method: NUnit.Framework.Constraints.Constraint.ToString
::CsString* NUnit::Framework::Constraints::Constraint::ToString() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "ToString"));
}
// Autogenerated method: NUnit.Framework.Constraints.Constraint.NUnit.Framework.Constraints.IResolveConstraint.Resolve
NUnit::Framework::Constraints::IConstraint* NUnit::Framework::Constraints::Constraint::NUnit_Framework_Constraints_IResolveConstraint_Resolve() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<NUnit::Framework::Constraints::IConstraint*>(this, "NUnit.Framework.Constraints.IResolveConstraint.Resolve"));
}
