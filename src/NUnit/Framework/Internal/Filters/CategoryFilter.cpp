// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: NUnit.Framework.Internal.Filters.CategoryFilter
#include "NUnit/Framework/Internal/Filters/CategoryFilter.hpp"
// Including type: NUnit.Framework.Interfaces.ITest
#include "NUnit/Framework/Interfaces/ITest.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: NUnit.Framework.Internal.Filters.CategoryFilter..ctor
NUnit::Framework::Internal::Filters::CategoryFilter* NUnit::Framework::Internal::Filters::CategoryFilter::New_ctor(::CsString* name) {
  return (CategoryFilter*)THROW_UNLESS(il2cpp_utils::New("NUnit.Framework.Internal.Filters", "CategoryFilter", name));
}
// Autogenerated method: NUnit.Framework.Internal.Filters.CategoryFilter.Match
bool NUnit::Framework::Internal::Filters::CategoryFilter::Match(NUnit::Framework::Interfaces::ITest* test) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<bool>(this, "Match", test));
}
// Autogenerated method: NUnit.Framework.Internal.Filters.CategoryFilter.get_ElementName
::CsString* NUnit::Framework::Internal::Filters::CategoryFilter::get_ElementName() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_ElementName"));
}
