// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: UnityEngine.TestTools.Logging.LogEvent
#include "UnityEngine/TestTools/Logging/LogEvent.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: UnityEngine.TestTools.Logging.LogEvent.get_Message
::CsString* UnityEngine::TestTools::Logging::LogEvent::get_Message() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_Message"));
}
// Autogenerated method: UnityEngine.TestTools.Logging.LogEvent.set_Message
void UnityEngine::TestTools::Logging::LogEvent::set_Message(::CsString* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_Message", value));
}
// Autogenerated method: UnityEngine.TestTools.Logging.LogEvent.get_StackTrace
::CsString* UnityEngine::TestTools::Logging::LogEvent::get_StackTrace() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_StackTrace"));
}
// Autogenerated method: UnityEngine.TestTools.Logging.LogEvent.set_StackTrace
void UnityEngine::TestTools::Logging::LogEvent::set_StackTrace(::CsString* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_StackTrace", value));
}
// Autogenerated method: UnityEngine.TestTools.Logging.LogEvent.get_LogType
UnityEngine::LogType UnityEngine::TestTools::Logging::LogEvent::get_LogType() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<UnityEngine::LogType>(this, "get_LogType"));
}
// Autogenerated method: UnityEngine.TestTools.Logging.LogEvent.set_LogType
void UnityEngine::TestTools::Logging::LogEvent::set_LogType(UnityEngine::LogType value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_LogType", value));
}
// Autogenerated method: UnityEngine.TestTools.Logging.LogEvent.get_IsHandled
bool UnityEngine::TestTools::Logging::LogEvent::get_IsHandled() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<bool>(this, "get_IsHandled"));
}
// Autogenerated method: UnityEngine.TestTools.Logging.LogEvent.set_IsHandled
void UnityEngine::TestTools::Logging::LogEvent::set_IsHandled(bool value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_IsHandled", value));
}
// Autogenerated method: UnityEngine.TestTools.Logging.LogEvent.ToString
::CsString* UnityEngine::TestTools::Logging::LogEvent::ToString() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "ToString"));
}
// Autogenerated method: UnityEngine.TestTools.Logging.LogEvent..ctor
UnityEngine::TestTools::Logging::LogEvent* UnityEngine::TestTools::Logging::LogEvent::New_ctor() {
  return (LogEvent*)THROW_UNLESS(il2cpp_utils::New("UnityEngine.TestTools.Logging", "LogEvent"));
}
