// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: UnityEngine.Logger
#include "UnityEngine/Logger.hpp"
// Including type: UnityEngine.Object
#include "UnityEngine/Object.hpp"
// Including type: System.Exception
#include "System/Exception.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: UnityEngine.Logger..ctor
UnityEngine::Logger* UnityEngine::Logger::New_ctor(UnityEngine::ILogHandler* logHandler) {
  return (Logger*)THROW_UNLESS(il2cpp_utils::New("UnityEngine", "Logger", logHandler));
}
// Autogenerated method: UnityEngine.Logger.set_logHandler
void UnityEngine::Logger::set_logHandler(UnityEngine::ILogHandler* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_logHandler", value));
}
// Autogenerated method: UnityEngine.Logger.get_logEnabled
bool UnityEngine::Logger::get_logEnabled() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<bool>(this, "get_logEnabled"));
}
// Autogenerated method: UnityEngine.Logger.set_logEnabled
void UnityEngine::Logger::set_logEnabled(bool value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_logEnabled", value));
}
// Autogenerated method: UnityEngine.Logger.get_filterLogType
UnityEngine::LogType UnityEngine::Logger::get_filterLogType() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<UnityEngine::LogType>(this, "get_filterLogType"));
}
// Autogenerated method: UnityEngine.Logger.set_filterLogType
void UnityEngine::Logger::set_filterLogType(UnityEngine::LogType value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_filterLogType", value));
}
// Autogenerated method: UnityEngine.Logger.IsLogTypeAllowed
bool UnityEngine::Logger::IsLogTypeAllowed(UnityEngine::LogType logType) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<bool>(this, "IsLogTypeAllowed", logType));
}
// Autogenerated method: UnityEngine.Logger.GetString
::CsString* UnityEngine::Logger::GetString(::CsObject* message) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>("UnityEngine", "Logger", "GetString", message));
}
// Autogenerated method: UnityEngine.Logger.get_logHandler
UnityEngine::ILogHandler* UnityEngine::Logger::get_logHandler() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<UnityEngine::ILogHandler*>(this, "get_logHandler"));
}
// Autogenerated method: UnityEngine.Logger.Log
void UnityEngine::Logger::Log(UnityEngine::LogType logType, ::CsObject* message) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "Log", logType, message));
}
// Autogenerated method: UnityEngine.Logger.Log
void UnityEngine::Logger::Log(UnityEngine::LogType logType, ::CsObject* message, UnityEngine::Object* context) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "Log", logType, message, context));
}
// Autogenerated method: UnityEngine.Logger.LogFormat
void UnityEngine::Logger::LogFormat(UnityEngine::LogType logType, ::CsString* format, ::Array<::CsObject*>* args) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "LogFormat", logType, format, args));
}
// Autogenerated method: UnityEngine.Logger.LogFormat
void UnityEngine::Logger::LogFormat(UnityEngine::LogType logType, UnityEngine::Object* context, ::CsString* format, ::Array<::CsObject*>* args) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "LogFormat", logType, context, format, args));
}
// Autogenerated method: UnityEngine.Logger.LogException
void UnityEngine::Logger::LogException(System::Exception* exception, UnityEngine::Object* context) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "LogException", exception, context));
}
