// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: JetBrains.Annotations.MacroAttribute
#include "JetBrains/Annotations/MacroAttribute.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: JetBrains.Annotations.MacroAttribute.get_Expression
::CsString* JetBrains::Annotations::MacroAttribute::get_Expression() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_Expression"));
}
// Autogenerated method: JetBrains.Annotations.MacroAttribute.set_Expression
void JetBrains::Annotations::MacroAttribute::set_Expression(::CsString* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_Expression", value));
}
// Autogenerated method: JetBrains.Annotations.MacroAttribute.get_Editable
int JetBrains::Annotations::MacroAttribute::get_Editable() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<int>(this, "get_Editable"));
}
// Autogenerated method: JetBrains.Annotations.MacroAttribute.set_Editable
void JetBrains::Annotations::MacroAttribute::set_Editable(int value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_Editable", value));
}
// Autogenerated method: JetBrains.Annotations.MacroAttribute.get_Target
::CsString* JetBrains::Annotations::MacroAttribute::get_Target() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_Target"));
}
// Autogenerated method: JetBrains.Annotations.MacroAttribute.set_Target
void JetBrains::Annotations::MacroAttribute::set_Target(::CsString* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_Target", value));
}
// Autogenerated method: JetBrains.Annotations.MacroAttribute..ctor
JetBrains::Annotations::MacroAttribute* JetBrains::Annotations::MacroAttribute::New_ctor() {
  return (MacroAttribute*)THROW_UNLESS(il2cpp_utils::New("JetBrains.Annotations", "MacroAttribute"));
}
