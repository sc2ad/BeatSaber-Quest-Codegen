// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: System.Exception
#include "System/Exception.hpp"
// Including type: System.Exception/ExceptionMessageKind
#include "System/Exception_ExceptionMessageKind.hpp"
// Including type: System.Collections.IDictionary
#include "System/Collections/IDictionary.hpp"
// Including type: System.Runtime.Serialization.SafeSerializationManager
#include "System/Runtime/Serialization/SafeSerializationManager.hpp"
// Including type: System.Diagnostics.StackTrace
#include "System/Diagnostics/StackTrace.hpp"
// Including type: System.IntPtr
#include "System/IntPtr.hpp"
// Including type: System.Runtime.Serialization.SerializationInfo
#include "System/Runtime/Serialization/SerializationInfo.hpp"
// Including type: System.Runtime.Serialization.StreamingContext
#include "System/Runtime/Serialization/StreamingContext.hpp"
// Including type: System.Runtime.ExceptionServices.ExceptionDispatchInfo
#include "System/Runtime/ExceptionServices/ExceptionDispatchInfo.hpp"
// Including type: System.Type
#include "System/Type.hpp"
#include "extern/beatsaber-hook/shared/utils/il2cpp-utils.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated static field getter
// Get static field: static private System.Object s_EDILock
::CsObject* System::Exception::_get_s_EDILock() {
  return THROW_UNLESS(il2cpp_utils::GetFieldValue<::CsObject*>("System", "Exception", "s_EDILock"));
}
// Autogenerated static field setter
// Set static field: static private System.Object s_EDILock
void System::Exception::_set_s_EDILock(::CsObject* value) {
  THROW_UNLESS(il2cpp_utils::SetFieldValue("System", "Exception", "s_EDILock", value));
}
// Autogenerated static field getter
// Get static field: static private System.Int32 _COMPlusExceptionCode
int System::Exception::_get__COMPlusExceptionCode() {
  return THROW_UNLESS(il2cpp_utils::GetFieldValue<int>("System", "Exception", "_COMPlusExceptionCode"));
}
// Autogenerated static field setter
// Set static field: static private System.Int32 _COMPlusExceptionCode
void System::Exception::_set__COMPlusExceptionCode(int value) {
  THROW_UNLESS(il2cpp_utils::SetFieldValue("System", "Exception", "_COMPlusExceptionCode", value));
}
// Autogenerated method: System.Exception.Init
void System::Exception::Init() {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "Init"));
}
// Autogenerated method: System.Exception..ctor
System::Exception* System::Exception::New_ctor(::CsString* message) {
  return (Exception*)THROW_UNLESS(il2cpp_utils::New("System", "Exception", message));
}
// Autogenerated method: System.Exception..ctor
System::Exception* System::Exception::New_ctor(::CsString* message, System::Exception* innerException) {
  return (Exception*)THROW_UNLESS(il2cpp_utils::New("System", "Exception", message, innerException));
}
// Autogenerated method: System.Exception..ctor
System::Exception* System::Exception::New_ctor(System::Runtime::Serialization::SerializationInfo* info, System::Runtime::Serialization::StreamingContext context) {
  return (Exception*)THROW_UNLESS(il2cpp_utils::New("System", "Exception", info, context));
}
// Autogenerated method: System.Exception.get_Message
::CsString* System::Exception::get_Message() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_Message"));
}
// Autogenerated method: System.Exception.get_Data
System::Collections::IDictionary* System::Exception::get_Data() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<System::Collections::IDictionary*>(this, "get_Data"));
}
// Autogenerated method: System.Exception.IsImmutableAgileException
bool System::Exception::IsImmutableAgileException(System::Exception* e) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<bool>("System", "Exception", "IsImmutableAgileException", e));
}
// Autogenerated method: System.Exception.GetClassName
::CsString* System::Exception::GetClassName() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "GetClassName"));
}
// Autogenerated method: System.Exception.get_InnerException
System::Exception* System::Exception::get_InnerException() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<System::Exception*>(this, "get_InnerException"));
}
// Autogenerated method: System.Exception.get_StackTrace
::CsString* System::Exception::get_StackTrace() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_StackTrace"));
}
// Autogenerated method: System.Exception.GetStackTrace
::CsString* System::Exception::GetStackTrace(bool needFileInfo) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "GetStackTrace", needFileInfo));
}
// Autogenerated method: System.Exception.SetErrorCode
void System::Exception::SetErrorCode(int hr) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "SetErrorCode", hr));
}
// Autogenerated method: System.Exception.get_Source
::CsString* System::Exception::get_Source() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_Source"));
}
// Autogenerated method: System.Exception.ToString
::CsString* System::Exception::ToString(bool needFileLineInfo, bool needMessage) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "ToString", needFileLineInfo, needMessage));
}
// Autogenerated method: System.Exception.OnDeserialized
void System::Exception::OnDeserialized(System::Runtime::Serialization::StreamingContext context) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "OnDeserialized", context));
}
// Autogenerated method: System.Exception.InternalPreserveStackTrace
void System::Exception::InternalPreserveStackTrace() {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "InternalPreserveStackTrace"));
}
// Autogenerated method: System.Exception.StripFileInfo
::CsString* System::Exception::StripFileInfo(::CsString* stackTrace, bool isRemoteStackTrace) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "StripFileInfo", stackTrace, isRemoteStackTrace));
}
// Autogenerated method: System.Exception.RestoreExceptionDispatchInfo
void System::Exception::RestoreExceptionDispatchInfo(System::Runtime::ExceptionServices::ExceptionDispatchInfo* exceptionDispatchInfo) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "RestoreExceptionDispatchInfo", exceptionDispatchInfo));
}
// Autogenerated method: System.Exception.get_HResult
int System::Exception::get_HResult() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<int>(this, "get_HResult"));
}
// Autogenerated method: System.Exception.set_HResult
void System::Exception::set_HResult(int value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_HResult", value));
}
// Autogenerated method: System.Exception.GetMessageFromNativeResources
::CsString* System::Exception::GetMessageFromNativeResources(System::Exception::ExceptionMessageKind kind) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>("System", "Exception", "GetMessageFromNativeResources", kind));
}
// Autogenerated method: System.Exception.FixRemotingException
System::Exception* System::Exception::FixRemotingException() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<System::Exception*>(this, "FixRemotingException"));
}
// Autogenerated method: System.Exception.ReportUnhandledException
void System::Exception::ReportUnhandledException(System::Exception* exception) {
  THROW_UNLESS(il2cpp_utils::RunMethod("System", "Exception", "ReportUnhandledException", exception));
}
// Autogenerated method: System.Exception..cctor
void System::Exception::_cctor() {
  THROW_UNLESS(il2cpp_utils::RunMethod("System", "Exception", ".cctor"));
}
// Autogenerated method: System.Exception..ctor
System::Exception* System::Exception::New_ctor() {
  return (Exception*)THROW_UNLESS(il2cpp_utils::New("System", "Exception"));
}
// Autogenerated method: System.Exception.ToString
::CsString* System::Exception::ToString() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "ToString"));
}
// Autogenerated method: System.Exception.GetObjectData
void System::Exception::GetObjectData(System::Runtime::Serialization::SerializationInfo* info, System::Runtime::Serialization::StreamingContext context) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "GetObjectData", info, context));
}
void System::Exception::System_Runtime_Serialization_ISerializable_GetObjectData(System::Runtime::Serialization::SerializationInfo* info, System::Runtime::Serialization::StreamingContext context) {
  GetObjectData(info, context);
}
// Autogenerated method: System.Exception.GetType
System::Type* System::Exception::GetType() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<System::Type*>(this, "GetType"));
}
