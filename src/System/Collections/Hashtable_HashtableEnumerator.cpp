// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: System.Collections.Hashtable/HashtableEnumerator
#include "System/Collections/Hashtable_HashtableEnumerator.hpp"
// Including type: System.Collections.DictionaryEntry
#include "System/Collections/DictionaryEntry.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: System.Collections.Hashtable/HashtableEnumerator..ctor
System::Collections::Hashtable::HashtableEnumerator* System::Collections::Hashtable::HashtableEnumerator::New_ctor(System::Collections::Hashtable* hashtable, int getObjRetType) {
  return (Hashtable::HashtableEnumerator*)THROW_UNLESS(il2cpp_utils::New("System.Collections", "Hashtable/HashtableEnumerator", hashtable, getObjRetType));
}
// Autogenerated method: System.Collections.Hashtable/HashtableEnumerator.Clone
::CsObject* System::Collections::Hashtable::HashtableEnumerator::Clone() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsObject*>(this, "Clone"));
}
// Autogenerated method: System.Collections.Hashtable/HashtableEnumerator.get_Key
::CsObject* System::Collections::Hashtable::HashtableEnumerator::get_Key() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsObject*>(this, "get_Key"));
}
::CsObject* System::Collections::Hashtable::HashtableEnumerator::System_Collections_IDictionaryEnumerator_get_Key() {
  return get_Key();
}
// Autogenerated method: System.Collections.Hashtable/HashtableEnumerator.MoveNext
bool System::Collections::Hashtable::HashtableEnumerator::MoveNext() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<bool>(this, "MoveNext"));
}
// Autogenerated method: System.Collections.Hashtable/HashtableEnumerator.get_Entry
System::Collections::DictionaryEntry System::Collections::Hashtable::HashtableEnumerator::get_Entry() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<System::Collections::DictionaryEntry>(this, "get_Entry"));
}
System::Collections::DictionaryEntry System::Collections::Hashtable::HashtableEnumerator::System_Collections_IDictionaryEnumerator_get_Entry() {
  return get_Entry();
}
// Autogenerated method: System.Collections.Hashtable/HashtableEnumerator.get_Current
::CsObject* System::Collections::Hashtable::HashtableEnumerator::get_Current() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsObject*>(this, "get_Current"));
}
::CsObject* System::Collections::Hashtable::HashtableEnumerator::System_Collections_IEnumerator_get_Current() {
  return get_Current();
}
// Autogenerated method: System.Collections.Hashtable/HashtableEnumerator.get_Value
::CsObject* System::Collections::Hashtable::HashtableEnumerator::get_Value() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsObject*>(this, "get_Value"));
}
::CsObject* System::Collections::Hashtable::HashtableEnumerator::System_Collections_IDictionaryEnumerator_get_Value() {
  return get_Value();
}
// Autogenerated method: System.Collections.Hashtable/HashtableEnumerator.Reset
void System::Collections::Hashtable::HashtableEnumerator::Reset() {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "Reset"));
}
void System::Collections::Hashtable::HashtableEnumerator::System_Collections_IEnumerator_Reset() {
  Reset();
}
