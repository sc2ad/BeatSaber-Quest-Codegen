// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: System.Collections.Generic.InternalStringComparer
#include "System/Collections/Generic/InternalStringComparer.hpp"
#include "extern/beatsaber-hook/shared/utils/il2cpp-utils.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: System.Collections.Generic.InternalStringComparer.GetHashCode
int System::Collections::Generic::InternalStringComparer::GetHashCode(::CsString* obj) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<int>(this, "GetHashCode", obj));
}
// Autogenerated method: System.Collections.Generic.InternalStringComparer.Equals
bool System::Collections::Generic::InternalStringComparer::Equals(::CsString* x, ::CsString* y) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<bool>(this, "Equals", x, y));
}
// Autogenerated method: System.Collections.Generic.InternalStringComparer.IndexOf
int System::Collections::Generic::InternalStringComparer::IndexOf(::Array<::CsString*>* array, ::CsString* value, int startIndex, int count) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<int>(this, "IndexOf", array, value, startIndex, count));
}
// Autogenerated method: System.Collections.Generic.InternalStringComparer..ctor
System::Collections::Generic::InternalStringComparer* System::Collections::Generic::InternalStringComparer::New_ctor() {
  return (InternalStringComparer*)THROW_UNLESS(il2cpp_utils::New("System.Collections.Generic", "InternalStringComparer"));
}
