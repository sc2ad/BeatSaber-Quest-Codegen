// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: System.Runtime.Remoting.Messaging.ArgInfo
#include "System/Runtime/Remoting/Messaging/ArgInfo.hpp"
// Including type: System.Reflection.MethodBase
#include "System/Reflection/MethodBase.hpp"
// Including type: System.Runtime.Remoting.Messaging.ArgInfoType
#include "System/Runtime/Remoting/Messaging/ArgInfoType.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: System.Runtime.Remoting.Messaging.ArgInfo..ctor
System::Runtime::Remoting::Messaging::ArgInfo* System::Runtime::Remoting::Messaging::ArgInfo::New_ctor(System::Reflection::MethodBase* method, System::Runtime::Remoting::Messaging::ArgInfoType type) {
  return (ArgInfo*)THROW_UNLESS(il2cpp_utils::New("System.Runtime.Remoting.Messaging", "ArgInfo", method, type));
}
// Autogenerated method: System.Runtime.Remoting.Messaging.ArgInfo.GetInOutArgs
::Array<::CsObject*>* System::Runtime::Remoting::Messaging::ArgInfo::GetInOutArgs(::Array<::CsObject*>* args) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::Array<::CsObject*>*>(this, "GetInOutArgs", args));
}
