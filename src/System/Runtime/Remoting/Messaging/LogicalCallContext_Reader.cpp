// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: System.Runtime.Remoting.Messaging.LogicalCallContext/Reader
#include "System/Runtime/Remoting/Messaging/LogicalCallContext_Reader.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: System.Runtime.Remoting.Messaging.LogicalCallContext/Reader..ctor
System::Runtime::Remoting::Messaging::LogicalCallContext::Reader* System::Runtime::Remoting::Messaging::LogicalCallContext::Reader::New_ctor(System::Runtime::Remoting::Messaging::LogicalCallContext* ctx) {
  return (LogicalCallContext::Reader*)THROW_UNLESS(il2cpp_utils::New("System.Runtime.Remoting.Messaging", "LogicalCallContext/Reader", ctx));
}
// Autogenerated method: System.Runtime.Remoting.Messaging.LogicalCallContext/Reader.get_IsNull
bool System::Runtime::Remoting::Messaging::LogicalCallContext::Reader::get_IsNull() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<bool>(*this, "get_IsNull"));
}
// Autogenerated method: System.Runtime.Remoting.Messaging.LogicalCallContext/Reader.get_HasInfo
bool System::Runtime::Remoting::Messaging::LogicalCallContext::Reader::get_HasInfo() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<bool>(*this, "get_HasInfo"));
}
// Autogenerated method: System.Runtime.Remoting.Messaging.LogicalCallContext/Reader.Clone
System::Runtime::Remoting::Messaging::LogicalCallContext* System::Runtime::Remoting::Messaging::LogicalCallContext::Reader::Clone() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<System::Runtime::Remoting::Messaging::LogicalCallContext*>(*this, "Clone"));
}
// Autogenerated method: System.Runtime.Remoting.Messaging.LogicalCallContext/Reader.GetData
::CsObject* System::Runtime::Remoting::Messaging::LogicalCallContext::Reader::GetData(::CsString* name) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsObject*>(*this, "GetData", name));
}
