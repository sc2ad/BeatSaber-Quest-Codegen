// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: Oculus.Platform.RichPresenceOptions
#include "Oculus/Platform/RichPresenceOptions.hpp"
// Including type: System.DateTime
#include "System/DateTime.hpp"
// Including type: Oculus.Platform.RichPresenceExtraContext
#include "Oculus/Platform/RichPresenceExtraContext.hpp"
#include "extern/beatsaber-hook/shared/utils/il2cpp-utils.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: Oculus.Platform.RichPresenceOptions.SetApiName
void Oculus::Platform::RichPresenceOptions::SetApiName(::CsString* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "SetApiName", value));
}
// Autogenerated method: Oculus.Platform.RichPresenceOptions.SetArgs
void Oculus::Platform::RichPresenceOptions::SetArgs(::CsString* key, ::CsString* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "SetArgs", key, value));
}
// Autogenerated method: Oculus.Platform.RichPresenceOptions.ClearArgs
void Oculus::Platform::RichPresenceOptions::ClearArgs() {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "ClearArgs"));
}
// Autogenerated method: Oculus.Platform.RichPresenceOptions.SetCurrentCapacity
void Oculus::Platform::RichPresenceOptions::SetCurrentCapacity(uint value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "SetCurrentCapacity", value));
}
// Autogenerated method: Oculus.Platform.RichPresenceOptions.SetDeeplinkMessageOverride
void Oculus::Platform::RichPresenceOptions::SetDeeplinkMessageOverride(::CsString* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "SetDeeplinkMessageOverride", value));
}
// Autogenerated method: Oculus.Platform.RichPresenceOptions.SetEndTime
void Oculus::Platform::RichPresenceOptions::SetEndTime(System::DateTime value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "SetEndTime", value));
}
// Autogenerated method: Oculus.Platform.RichPresenceOptions.SetExtraContext
void Oculus::Platform::RichPresenceOptions::SetExtraContext(Oculus::Platform::RichPresenceExtraContext value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "SetExtraContext", value));
}
// Autogenerated method: Oculus.Platform.RichPresenceOptions.SetIsIdle
void Oculus::Platform::RichPresenceOptions::SetIsIdle(bool value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "SetIsIdle", value));
}
// Autogenerated method: Oculus.Platform.RichPresenceOptions.SetIsJoinable
void Oculus::Platform::RichPresenceOptions::SetIsJoinable(bool value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "SetIsJoinable", value));
}
// Autogenerated method: Oculus.Platform.RichPresenceOptions.SetJoinableId
void Oculus::Platform::RichPresenceOptions::SetJoinableId(::CsString* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "SetJoinableId", value));
}
// Autogenerated method: Oculus.Platform.RichPresenceOptions.SetMaxCapacity
void Oculus::Platform::RichPresenceOptions::SetMaxCapacity(uint value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "SetMaxCapacity", value));
}
// Autogenerated method: Oculus.Platform.RichPresenceOptions.SetStartTime
void Oculus::Platform::RichPresenceOptions::SetStartTime(System::DateTime value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "SetStartTime", value));
}
// Autogenerated method: Oculus.Platform.RichPresenceOptions..ctor
Oculus::Platform::RichPresenceOptions* Oculus::Platform::RichPresenceOptions::New_ctor() {
  return (RichPresenceOptions*)THROW_UNLESS(il2cpp_utils::New("Oculus.Platform", "RichPresenceOptions"));
}
// Autogenerated method: Oculus.Platform.RichPresenceOptions.Finalize
void Oculus::Platform::RichPresenceOptions::Finalize() {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "Finalize"));
}
