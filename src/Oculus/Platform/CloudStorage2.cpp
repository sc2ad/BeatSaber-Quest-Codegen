// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: Oculus.Platform.CloudStorage2
#include "Oculus/Platform/CloudStorage2.hpp"
// Including type: Oculus.Platform.Request`1
#include "Oculus/Platform/Request_1.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: Oculus.Platform.CloudStorage2.GetUserDirectoryPath
Oculus::Platform::Request_1<::CsString*>* Oculus::Platform::CloudStorage2::GetUserDirectoryPath() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<Oculus::Platform::Request_1<::CsString*>*>("Oculus.Platform", "CloudStorage2", "GetUserDirectoryPath"));
}
