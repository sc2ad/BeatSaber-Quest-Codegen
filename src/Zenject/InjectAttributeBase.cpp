// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: Zenject.InjectAttributeBase
#include "Zenject/InjectAttributeBase.hpp"
#include "extern/beatsaber-hook/shared/utils/il2cpp-utils.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: Zenject.InjectAttributeBase.get_Optional
bool Zenject::InjectAttributeBase::get_Optional() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<bool>(this, "get_Optional"));
}
// Autogenerated method: Zenject.InjectAttributeBase.set_Optional
void Zenject::InjectAttributeBase::set_Optional(bool value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_Optional", value));
}
// Autogenerated method: Zenject.InjectAttributeBase.get_Id
::CsObject* Zenject::InjectAttributeBase::get_Id() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsObject*>(this, "get_Id"));
}
// Autogenerated method: Zenject.InjectAttributeBase.set_Id
void Zenject::InjectAttributeBase::set_Id(::CsObject* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_Id", value));
}
// Autogenerated method: Zenject.InjectAttributeBase.get_Source
Zenject::InjectSources Zenject::InjectAttributeBase::get_Source() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<Zenject::InjectSources>(this, "get_Source"));
}
// Autogenerated method: Zenject.InjectAttributeBase.set_Source
void Zenject::InjectAttributeBase::set_Source(Zenject::InjectSources value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_Source", value));
}
// Autogenerated method: Zenject.InjectAttributeBase..ctor
Zenject::InjectAttributeBase* Zenject::InjectAttributeBase::New_ctor() {
  return (InjectAttributeBase*)THROW_UNLESS(il2cpp_utils::New("Zenject", "InjectAttributeBase"));
}
