// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: Zenject.InjectContext
#include "Zenject/InjectContext.hpp"
// Including type: Zenject.InjectContext/<get_ParentContexts>d__52
#include "Zenject/InjectContext_-get_ParentContexts-d__52.hpp"
// Including type: Zenject.InjectContext/<get_ParentContextsAndSelf>d__54
#include "Zenject/InjectContext_-get_ParentContextsAndSelf-d__54.hpp"
// Including type: Zenject.InjectContext/<get_AllObjectTypes>d__56
#include "Zenject/InjectContext_-get_AllObjectTypes-d__56.hpp"
// Including type: System.Type
#include "System/Type.hpp"
// Including type: Zenject.DiContainer
#include "Zenject/DiContainer.hpp"
// Including type: System.Collections.Generic.IEnumerable`1
#include "System/Collections/Generic/IEnumerable_1.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: Zenject.InjectContext..ctor
Zenject::InjectContext* Zenject::InjectContext::New_ctor(Zenject::DiContainer* container, System::Type* memberType) {
  return (InjectContext*)THROW_UNLESS(il2cpp_utils::New("Zenject", "InjectContext", container, memberType));
}
// Autogenerated method: Zenject.InjectContext..ctor
Zenject::InjectContext* Zenject::InjectContext::New_ctor(Zenject::DiContainer* container, System::Type* memberType, ::CsObject* identifier) {
  return (InjectContext*)THROW_UNLESS(il2cpp_utils::New("Zenject", "InjectContext", container, memberType, identifier));
}
// Autogenerated method: Zenject.InjectContext..ctor
Zenject::InjectContext* Zenject::InjectContext::New_ctor(Zenject::DiContainer* container, System::Type* memberType, ::CsObject* identifier, bool optional) {
  return (InjectContext*)THROW_UNLESS(il2cpp_utils::New("Zenject", "InjectContext", container, memberType, identifier, optional));
}
// Autogenerated method: Zenject.InjectContext.Reset
void Zenject::InjectContext::Reset() {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "Reset"));
}
// Autogenerated method: Zenject.InjectContext.get_BindingId
Zenject::BindingId Zenject::InjectContext::get_BindingId() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<Zenject::BindingId>(this, "get_BindingId"));
}
// Autogenerated method: Zenject.InjectContext.get_ObjectType
System::Type* Zenject::InjectContext::get_ObjectType() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<System::Type*>(this, "get_ObjectType"));
}
// Autogenerated method: Zenject.InjectContext.set_ObjectType
void Zenject::InjectContext::set_ObjectType(System::Type* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_ObjectType", value));
}
// Autogenerated method: Zenject.InjectContext.get_ParentContext
Zenject::InjectContext* Zenject::InjectContext::get_ParentContext() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<Zenject::InjectContext*>(this, "get_ParentContext"));
}
// Autogenerated method: Zenject.InjectContext.set_ParentContext
void Zenject::InjectContext::set_ParentContext(Zenject::InjectContext* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_ParentContext", value));
}
// Autogenerated method: Zenject.InjectContext.get_ObjectInstance
::CsObject* Zenject::InjectContext::get_ObjectInstance() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsObject*>(this, "get_ObjectInstance"));
}
// Autogenerated method: Zenject.InjectContext.set_ObjectInstance
void Zenject::InjectContext::set_ObjectInstance(::CsObject* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_ObjectInstance", value));
}
// Autogenerated method: Zenject.InjectContext.get_Identifier
::CsObject* Zenject::InjectContext::get_Identifier() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsObject*>(this, "get_Identifier"));
}
// Autogenerated method: Zenject.InjectContext.set_Identifier
void Zenject::InjectContext::set_Identifier(::CsObject* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_Identifier", value));
}
// Autogenerated method: Zenject.InjectContext.get_MemberName
::CsString* Zenject::InjectContext::get_MemberName() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_MemberName"));
}
// Autogenerated method: Zenject.InjectContext.set_MemberName
void Zenject::InjectContext::set_MemberName(::CsString* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_MemberName", value));
}
// Autogenerated method: Zenject.InjectContext.get_MemberType
System::Type* Zenject::InjectContext::get_MemberType() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<System::Type*>(this, "get_MemberType"));
}
// Autogenerated method: Zenject.InjectContext.set_MemberType
void Zenject::InjectContext::set_MemberType(System::Type* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_MemberType", value));
}
// Autogenerated method: Zenject.InjectContext.get_Optional
bool Zenject::InjectContext::get_Optional() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<bool>(this, "get_Optional"));
}
// Autogenerated method: Zenject.InjectContext.set_Optional
void Zenject::InjectContext::set_Optional(bool value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_Optional", value));
}
// Autogenerated method: Zenject.InjectContext.get_SourceType
Zenject::InjectSources Zenject::InjectContext::get_SourceType() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<Zenject::InjectSources>(this, "get_SourceType"));
}
// Autogenerated method: Zenject.InjectContext.set_SourceType
void Zenject::InjectContext::set_SourceType(Zenject::InjectSources value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_SourceType", value));
}
// Autogenerated method: Zenject.InjectContext.get_ConcreteIdentifier
::CsObject* Zenject::InjectContext::get_ConcreteIdentifier() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsObject*>(this, "get_ConcreteIdentifier"));
}
// Autogenerated method: Zenject.InjectContext.set_ConcreteIdentifier
void Zenject::InjectContext::set_ConcreteIdentifier(::CsObject* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_ConcreteIdentifier", value));
}
// Autogenerated method: Zenject.InjectContext.get_FallBackValue
::CsObject* Zenject::InjectContext::get_FallBackValue() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsObject*>(this, "get_FallBackValue"));
}
// Autogenerated method: Zenject.InjectContext.set_FallBackValue
void Zenject::InjectContext::set_FallBackValue(::CsObject* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_FallBackValue", value));
}
// Autogenerated method: Zenject.InjectContext.get_Container
Zenject::DiContainer* Zenject::InjectContext::get_Container() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<Zenject::DiContainer*>(this, "get_Container"));
}
// Autogenerated method: Zenject.InjectContext.set_Container
void Zenject::InjectContext::set_Container(Zenject::DiContainer* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_Container", value));
}
// Autogenerated method: Zenject.InjectContext.get_ParentContexts
System::Collections::Generic::IEnumerable_1<Zenject::InjectContext*>* Zenject::InjectContext::get_ParentContexts() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<System::Collections::Generic::IEnumerable_1<Zenject::InjectContext*>*>(this, "get_ParentContexts"));
}
// Autogenerated method: Zenject.InjectContext.get_ParentContextsAndSelf
System::Collections::Generic::IEnumerable_1<Zenject::InjectContext*>* Zenject::InjectContext::get_ParentContextsAndSelf() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<System::Collections::Generic::IEnumerable_1<Zenject::InjectContext*>*>(this, "get_ParentContextsAndSelf"));
}
// Autogenerated method: Zenject.InjectContext.get_AllObjectTypes
System::Collections::Generic::IEnumerable_1<System::Type*>* Zenject::InjectContext::get_AllObjectTypes() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<System::Collections::Generic::IEnumerable_1<System::Type*>*>(this, "get_AllObjectTypes"));
}
// Autogenerated method: Zenject.InjectContext.CreateSubContext
Zenject::InjectContext* Zenject::InjectContext::CreateSubContext(System::Type* memberType) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<Zenject::InjectContext*>(this, "CreateSubContext", memberType));
}
// Autogenerated method: Zenject.InjectContext.CreateSubContext
Zenject::InjectContext* Zenject::InjectContext::CreateSubContext(System::Type* memberType, ::CsObject* identifier) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<Zenject::InjectContext*>(this, "CreateSubContext", memberType, identifier));
}
// Autogenerated method: Zenject.InjectContext.Clone
Zenject::InjectContext* Zenject::InjectContext::Clone() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<Zenject::InjectContext*>(this, "Clone"));
}
// Autogenerated method: Zenject.InjectContext.GetObjectGraphString
::CsString* Zenject::InjectContext::GetObjectGraphString() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "GetObjectGraphString"));
}
// Autogenerated method: Zenject.InjectContext..ctor
Zenject::InjectContext* Zenject::InjectContext::New_ctor() {
  return (InjectContext*)THROW_UNLESS(il2cpp_utils::New("Zenject", "InjectContext"));
}
// Autogenerated method: Zenject.InjectContext.Dispose
void Zenject::InjectContext::Dispose() {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "Dispose"));
}
void Zenject::InjectContext::System_IDisposable_Dispose() {
  Dispose();
}
