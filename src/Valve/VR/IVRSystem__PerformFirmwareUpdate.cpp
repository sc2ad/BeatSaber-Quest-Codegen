// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: Valve.VR.IVRSystem/_PerformFirmwareUpdate
#include "Valve/VR/IVRSystem__PerformFirmwareUpdate.hpp"
// Including type: Valve.VR.EVRFirmwareError
#include "Valve/VR/EVRFirmwareError.hpp"
// Including type: System.IAsyncResult
#include "System/IAsyncResult.hpp"
// Including type: System.AsyncCallback
#include "System/AsyncCallback.hpp"
#include "extern/beatsaber-hook/shared/utils/il2cpp-utils.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: Valve.VR.IVRSystem/_PerformFirmwareUpdate..ctor
Valve::VR::IVRSystem::_PerformFirmwareUpdate* Valve::VR::IVRSystem::_PerformFirmwareUpdate::New_ctor(::CsObject* object, System::IntPtr method) {
  return (IVRSystem::_PerformFirmwareUpdate*)THROW_UNLESS(il2cpp_utils::New("Valve.VR", "IVRSystem/_PerformFirmwareUpdate", object, method));
}
// Autogenerated method: Valve.VR.IVRSystem/_PerformFirmwareUpdate.Invoke
Valve::VR::EVRFirmwareError Valve::VR::IVRSystem::_PerformFirmwareUpdate::Invoke(uint unDeviceIndex) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<Valve::VR::EVRFirmwareError>(this, "Invoke", unDeviceIndex));
}
// Autogenerated method: Valve.VR.IVRSystem/_PerformFirmwareUpdate.BeginInvoke
System::IAsyncResult* Valve::VR::IVRSystem::_PerformFirmwareUpdate::BeginInvoke(uint unDeviceIndex, System::AsyncCallback* callback, ::CsObject* object) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<System::IAsyncResult*>(this, "BeginInvoke", unDeviceIndex, callback, object));
}
// Autogenerated method: Valve.VR.IVRSystem/_PerformFirmwareUpdate.EndInvoke
Valve::VR::EVRFirmwareError Valve::VR::IVRSystem::_PerformFirmwareUpdate::EndInvoke(System::IAsyncResult* result) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<Valve::VR::EVRFirmwareError>(this, "EndInvoke", result));
}
