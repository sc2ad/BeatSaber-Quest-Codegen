// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: MissionObjectiveTypeSO
#include "GlobalNamespace/MissionObjectiveTypeSO.hpp"
// Including type: ObjectiveValueFormatterSO
#include "GlobalNamespace/ObjectiveValueFormatterSO.hpp"
#include "extern/beatsaber-hook/shared/utils/il2cpp-utils.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: MissionObjectiveTypeSO.get_objectiveName
::CsString* GlobalNamespace::MissionObjectiveTypeSO::get_objectiveName() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_objectiveName"));
}
// Autogenerated method: MissionObjectiveTypeSO.get_objectiveNameLocalized
::CsString* GlobalNamespace::MissionObjectiveTypeSO::get_objectiveNameLocalized() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_objectiveNameLocalized"));
}
// Autogenerated method: MissionObjectiveTypeSO.get_noConditionValue
bool GlobalNamespace::MissionObjectiveTypeSO::get_noConditionValue() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<bool>(this, "get_noConditionValue"));
}
// Autogenerated method: MissionObjectiveTypeSO.get_objectiveValueFormater
GlobalNamespace::ObjectiveValueFormatterSO* GlobalNamespace::MissionObjectiveTypeSO::get_objectiveValueFormater() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<GlobalNamespace::ObjectiveValueFormatterSO*>(this, "get_objectiveValueFormater"));
}
// Autogenerated method: MissionObjectiveTypeSO..ctor
GlobalNamespace::MissionObjectiveTypeSO* GlobalNamespace::MissionObjectiveTypeSO::New_ctor() {
  return (MissionObjectiveTypeSO*)THROW_UNLESS(il2cpp_utils::New("", "MissionObjectiveTypeSO"));
}
