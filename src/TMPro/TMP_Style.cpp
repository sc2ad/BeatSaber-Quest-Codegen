// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: TMPro.TMP_Style
#include "TMPro/TMP_Style.hpp"
#include "extern/beatsaber-hook/shared/utils/il2cpp-utils.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: TMPro.TMP_Style.get_name
::CsString* TMPro::TMP_Style::get_name() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_name"));
}
// Autogenerated method: TMPro.TMP_Style.set_name
void TMPro::TMP_Style::set_name(::CsString* value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_name", value));
}
// Autogenerated method: TMPro.TMP_Style.get_hashCode
int TMPro::TMP_Style::get_hashCode() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<int>(this, "get_hashCode"));
}
// Autogenerated method: TMPro.TMP_Style.set_hashCode
void TMPro::TMP_Style::set_hashCode(int value) {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "set_hashCode", value));
}
// Autogenerated method: TMPro.TMP_Style.get_styleOpeningDefinition
::CsString* TMPro::TMP_Style::get_styleOpeningDefinition() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_styleOpeningDefinition"));
}
// Autogenerated method: TMPro.TMP_Style.get_styleClosingDefinition
::CsString* TMPro::TMP_Style::get_styleClosingDefinition() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(this, "get_styleClosingDefinition"));
}
// Autogenerated method: TMPro.TMP_Style.get_styleOpeningTagArray
::Array<int>* TMPro::TMP_Style::get_styleOpeningTagArray() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::Array<int>*>(this, "get_styleOpeningTagArray"));
}
// Autogenerated method: TMPro.TMP_Style.get_styleClosingTagArray
::Array<int>* TMPro::TMP_Style::get_styleClosingTagArray() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::Array<int>*>(this, "get_styleClosingTagArray"));
}
// Autogenerated method: TMPro.TMP_Style.RefreshStyle
void TMPro::TMP_Style::RefreshStyle() {
  THROW_UNLESS(il2cpp_utils::RunMethod(this, "RefreshStyle"));
}
// Autogenerated method: TMPro.TMP_Style..ctor
TMPro::TMP_Style* TMPro::TMP_Style::New_ctor() {
  return (TMP_Style*)THROW_UNLESS(il2cpp_utils::New("TMPro", "TMP_Style"));
}
