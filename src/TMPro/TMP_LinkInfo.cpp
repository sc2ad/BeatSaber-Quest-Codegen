// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: TMPro.TMP_LinkInfo
#include "TMPro/TMP_LinkInfo.hpp"
// Including type: TMPro.TMP_Text
#include "TMPro/TMP_Text.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: TMPro.TMP_LinkInfo.SetLinkID
void TMPro::TMP_LinkInfo::SetLinkID(::Array<::Il2CppChar>* text, int startIndex, int length) {
  THROW_UNLESS(il2cpp_utils::RunMethod(*this, "SetLinkID", text, startIndex, length));
}
// Autogenerated method: TMPro.TMP_LinkInfo.GetLinkText
::CsString* TMPro::TMP_LinkInfo::GetLinkText() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(*this, "GetLinkText"));
}
// Autogenerated method: TMPro.TMP_LinkInfo.GetLinkID
::CsString* TMPro::TMP_LinkInfo::GetLinkID() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>(*this, "GetLinkID"));
}
