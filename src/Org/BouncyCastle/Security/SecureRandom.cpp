// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: Org.BouncyCastle.Security.SecureRandom
#include "Org/BouncyCastle/Security/SecureRandom.hpp"
// Including type: Org.BouncyCastle.Crypto.Prng.IRandomGenerator
#include "Org/BouncyCastle/Crypto/Prng/IRandomGenerator.hpp"
// Including type: Org.BouncyCastle.Crypto.Prng.DigestRandomGenerator
#include "Org/BouncyCastle/Crypto/Prng/DigestRandomGenerator.hpp"
#include "extern/beatsaber-hook/shared/utils/il2cpp-utils.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated static field getter
// Get static field: static private System.Int64 counter
int64_t Org::BouncyCastle::Security::SecureRandom::_get_counter() {
  static auto ___internal__logger = Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext("_get_counter");
  return THROW_UNLESS(il2cpp_utils::GetFieldValue<int64_t>("Org.BouncyCastle.Security", "SecureRandom", "counter"));
}
// Autogenerated static field setter
// Set static field: static private System.Int64 counter
void Org::BouncyCastle::Security::SecureRandom::_set_counter(int64_t value) {
  static auto ___internal__logger = Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext("_set_counter");
  THROW_UNLESS(il2cpp_utils::SetFieldValue("Org.BouncyCastle.Security", "SecureRandom", "counter", value));
}
// Autogenerated static field getter
// Get static field: static private readonly Org.BouncyCastle.Security.SecureRandom master
Org::BouncyCastle::Security::SecureRandom* Org::BouncyCastle::Security::SecureRandom::_get_master() {
  static auto ___internal__logger = Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext("_get_master");
  return THROW_UNLESS(il2cpp_utils::GetFieldValue<Org::BouncyCastle::Security::SecureRandom*>("Org.BouncyCastle.Security", "SecureRandom", "master"));
}
// Autogenerated static field setter
// Set static field: static private readonly Org.BouncyCastle.Security.SecureRandom master
void Org::BouncyCastle::Security::SecureRandom::_set_master(Org::BouncyCastle::Security::SecureRandom* value) {
  static auto ___internal__logger = Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext("_set_master");
  THROW_UNLESS(il2cpp_utils::SetFieldValue("Org.BouncyCastle.Security", "SecureRandom", "master", value));
}
// Autogenerated static field getter
// Get static field: static private readonly System.Double DoubleScale
double Org::BouncyCastle::Security::SecureRandom::_get_DoubleScale() {
  static auto ___internal__logger = Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext("_get_DoubleScale");
  return THROW_UNLESS(il2cpp_utils::GetFieldValue<double>("Org.BouncyCastle.Security", "SecureRandom", "DoubleScale"));
}
// Autogenerated static field setter
// Set static field: static private readonly System.Double DoubleScale
void Org::BouncyCastle::Security::SecureRandom::_set_DoubleScale(double value) {
  static auto ___internal__logger = Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext("_set_DoubleScale");
  THROW_UNLESS(il2cpp_utils::SetFieldValue("Org.BouncyCastle.Security", "SecureRandom", "DoubleScale", value));
}
// Autogenerated method: Org.BouncyCastle.Security.SecureRandom.NextCounterValue
int64_t Org::BouncyCastle::Security::SecureRandom::NextCounterValue() {
  static auto ___internal__logger = ::Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext("NextCounterValue");
  static auto* ___internal__method = THROW_UNLESS(::il2cpp_utils::FindMethod("Org.BouncyCastle.Security", "SecureRandom", il2cpp_utils::NoArgClass<int64_t>(), "NextCounterValue", {}, ::il2cpp_utils::ExtractTypes()));
  return ::il2cpp_utils::RunMethodThrow<int64_t, false>(static_cast<Il2CppClass*>(nullptr), ___internal__method);
}
// Autogenerated method: Org.BouncyCastle.Security.SecureRandom.get_Master
Org::BouncyCastle::Security::SecureRandom* Org::BouncyCastle::Security::SecureRandom::get_Master() {
  static auto ___internal__logger = ::Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext("get_Master");
  static auto* ___internal__method = THROW_UNLESS(::il2cpp_utils::FindMethod("Org.BouncyCastle.Security", "SecureRandom", il2cpp_utils::NoArgClass<Org::BouncyCastle::Security::SecureRandom*>(), "get_Master", {}, ::il2cpp_utils::ExtractTypes()));
  return ::il2cpp_utils::RunMethodThrow<Org::BouncyCastle::Security::SecureRandom*, false>(static_cast<Il2CppClass*>(nullptr), ___internal__method);
}
// Autogenerated method: Org.BouncyCastle.Security.SecureRandom.CreatePrng
Org::BouncyCastle::Crypto::Prng::DigestRandomGenerator* Org::BouncyCastle::Security::SecureRandom::CreatePrng(::Il2CppString* digestName, bool autoSeed) {
  static auto ___internal__logger = ::Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext("CreatePrng");
  static auto* ___internal__method = THROW_UNLESS(::il2cpp_utils::FindMethod("Org.BouncyCastle.Security", "SecureRandom", il2cpp_utils::NoArgClass<Org::BouncyCastle::Crypto::Prng::DigestRandomGenerator*>(), "CreatePrng", {}, ::il2cpp_utils::ExtractTypes(digestName, autoSeed)));
  return ::il2cpp_utils::RunMethodThrow<Org::BouncyCastle::Crypto::Prng::DigestRandomGenerator*, false>(static_cast<Il2CppClass*>(nullptr), ___internal__method, digestName, autoSeed);
}
// Autogenerated method: Org.BouncyCastle.Security.SecureRandom.GetNextBytes
::Array<uint8_t>* Org::BouncyCastle::Security::SecureRandom::GetNextBytes(Org::BouncyCastle::Security::SecureRandom* secureRandom, int length) {
  static auto ___internal__logger = ::Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext("GetNextBytes");
  static auto* ___internal__method = THROW_UNLESS(::il2cpp_utils::FindMethod("Org.BouncyCastle.Security", "SecureRandom", il2cpp_utils::NoArgClass<::Array<uint8_t>*>(), "GetNextBytes", {}, ::il2cpp_utils::ExtractTypes(secureRandom, length)));
  return ::il2cpp_utils::RunMethodThrow<::Array<uint8_t>*, false>(static_cast<Il2CppClass*>(nullptr), ___internal__method, secureRandom, length);
}
// Autogenerated method: Org.BouncyCastle.Security.SecureRandom..ctor
Org::BouncyCastle::Security::SecureRandom* Org::BouncyCastle::Security::SecureRandom::New_ctor(Org::BouncyCastle::Crypto::Prng::IRandomGenerator* generator) {
  static auto ___internal__logger = ::Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext(".ctor");
  return THROW_UNLESS(::il2cpp_utils::New<SecureRandom*>(generator));
}
// Autogenerated method: Org.BouncyCastle.Security.SecureRandom.NextInt
int Org::BouncyCastle::Security::SecureRandom::NextInt() {
  static auto ___internal__logger = ::Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext("NextInt");
  static auto* ___internal__method = THROW_UNLESS(::il2cpp_utils::FindMethod(this, il2cpp_utils::NoArgClass<int>(), "NextInt", {}, ::il2cpp_utils::ExtractTypes()));
  return ::il2cpp_utils::RunMethodThrow<int, false>(this, ___internal__method);
}
// Autogenerated method: Org.BouncyCastle.Security.SecureRandom..cctor
void Org::BouncyCastle::Security::SecureRandom::_cctor() {
  static auto ___internal__logger = ::Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext(".cctor");
  static auto* ___internal__method = THROW_UNLESS(::il2cpp_utils::FindMethod("Org.BouncyCastle.Security", "SecureRandom", il2cpp_utils::NoArgClass<void>(), ".cctor", {}, ::il2cpp_utils::ExtractTypes()));
  ::il2cpp_utils::RunMethodThrow<void, false>(static_cast<Il2CppClass*>(nullptr), ___internal__method);
}
// Autogenerated method: Org.BouncyCastle.Security.SecureRandom..ctor
Org::BouncyCastle::Security::SecureRandom* Org::BouncyCastle::Security::SecureRandom::New_ctor() {
  static auto ___internal__logger = ::Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext(".ctor");
  return THROW_UNLESS(::il2cpp_utils::New<SecureRandom*>());
}
// Autogenerated method: Org.BouncyCastle.Security.SecureRandom.Next
int Org::BouncyCastle::Security::SecureRandom::Next() {
  static auto ___internal__logger = ::Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext("Next");
  static auto* ___internal__method = THROW_UNLESS(::il2cpp_utils::FindMethod(this, il2cpp_utils::NoArgClass<int>(), "Next", {}, ::il2cpp_utils::ExtractTypes()));
  return ::il2cpp_utils::RunMethodThrow<int, false>(this, ___internal__method);
}
// Autogenerated method: Org.BouncyCastle.Security.SecureRandom.Next
int Org::BouncyCastle::Security::SecureRandom::Next(int maxValue) {
  static auto ___internal__logger = ::Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext("Next");
  static auto* ___internal__method = THROW_UNLESS(::il2cpp_utils::FindMethod(this, il2cpp_utils::NoArgClass<int>(), "Next", {}, ::il2cpp_utils::ExtractTypes(maxValue)));
  return ::il2cpp_utils::RunMethodThrow<int, false>(this, ___internal__method, maxValue);
}
// Autogenerated method: Org.BouncyCastle.Security.SecureRandom.Next
int Org::BouncyCastle::Security::SecureRandom::Next(int minValue, int maxValue) {
  static auto ___internal__logger = ::Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext("Next");
  static auto* ___internal__method = THROW_UNLESS(::il2cpp_utils::FindMethod(this, il2cpp_utils::NoArgClass<int>(), "Next", {}, ::il2cpp_utils::ExtractTypes(minValue, maxValue)));
  return ::il2cpp_utils::RunMethodThrow<int, false>(this, ___internal__method, minValue, maxValue);
}
// Autogenerated method: Org.BouncyCastle.Security.SecureRandom.NextBytes
void Org::BouncyCastle::Security::SecureRandom::NextBytes(::Array<uint8_t>* buf) {
  static auto ___internal__logger = ::Logger::get().WithContext("codegen").WithContext("Org::BouncyCastle::Security").WithContext("SecureRandom").WithContext("NextBytes");
  static auto* ___internal__method = THROW_UNLESS(::il2cpp_utils::FindMethod(this, il2cpp_utils::NoArgClass<void>(), "NextBytes", {}, ::il2cpp_utils::ExtractTypes(buf)));
  ::il2cpp_utils::RunMethodThrow<void, false>(this, ___internal__method, buf);
}
