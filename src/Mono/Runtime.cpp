// Autogenerated from CppSourceCreator
// Created by Sc2ad
// =========================================================================
// Begin includes
#include "extern/beatsaber-hook/shared/utils/typedefs.h"
// Including type: Mono.Runtime
#include "Mono/Runtime.hpp"
// Including type: System.Exception
#include "System/Exception.hpp"
#include "extern/beatsaber-hook/shared/utils/utils.h"
// Completed includes
// Autogenerated method: Mono.Runtime.mono_runtime_install_handlers
void Mono::Runtime::mono_runtime_install_handlers() {
  THROW_UNLESS(il2cpp_utils::RunMethod("Mono", "Runtime", "mono_runtime_install_handlers"));
}
// Autogenerated method: Mono.Runtime.InstallSignalHandlers
void Mono::Runtime::InstallSignalHandlers() {
  THROW_UNLESS(il2cpp_utils::RunMethod("Mono", "Runtime", "InstallSignalHandlers"));
}
// Autogenerated method: Mono.Runtime.mono_runtime_cleanup_handlers
void Mono::Runtime::mono_runtime_cleanup_handlers() {
  THROW_UNLESS(il2cpp_utils::RunMethod("Mono", "Runtime", "mono_runtime_cleanup_handlers"));
}
// Autogenerated method: Mono.Runtime.RemoveSignalHandlers
void Mono::Runtime::RemoveSignalHandlers() {
  THROW_UNLESS(il2cpp_utils::RunMethod("Mono", "Runtime", "RemoveSignalHandlers"));
}
// Autogenerated method: Mono.Runtime.GetDisplayName
::CsString* Mono::Runtime::GetDisplayName() {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>("Mono", "Runtime", "GetDisplayName"));
}
// Autogenerated method: Mono.Runtime.GetNativeStackTrace
::CsString* Mono::Runtime::GetNativeStackTrace(System::Exception* exception) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<::CsString*>("Mono", "Runtime", "GetNativeStackTrace", exception));
}
// Autogenerated method: Mono.Runtime.SetGCAllowSynchronousMajor
bool Mono::Runtime::SetGCAllowSynchronousMajor(bool flag) {
  return THROW_UNLESS(il2cpp_utils::RunMethod<bool>("Mono", "Runtime", "SetGCAllowSynchronousMajor", flag));
}
